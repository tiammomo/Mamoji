...
docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" stop backend frontend caddy prometheus >/dev/null 2>&1 || true
...
docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" stop minio >/dev/null
...
docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" up -d minio >/dev/null
docker compose -f "$COMPOSE_FILE" --env-file "$ENV_FILE" up -d backend frontend caddy prometheus >/dev/null
