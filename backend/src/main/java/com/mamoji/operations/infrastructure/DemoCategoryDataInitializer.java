...
/** Owns local category fixtures and is absent from production bootstrap contexts. */
@Component("categoryDataInitializer")
@ConditionalOnProperty(name = "mamoji.bootstrap.mode", havingValue = "demo", matchIfMissing = true)
@DependsOn("ledgerDataInitializer")
public class DemoCategoryDataInitializer {
    ...
    public DemoCategoryDataInitializer(CategoryRepository categories, CompanyRepository companies) {
    ...
    @PostConstruct
    void initialize() {
        List<Company> subjects = companies.findAll();
        subjects.stream()
