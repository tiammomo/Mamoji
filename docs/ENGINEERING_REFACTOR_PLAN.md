...
| P1 | `InMemoryStore` 名称与 JDBC 事实不符且责任过多 | 拆为 identity、operations、finance、budget、recurring 仓储 | 进行中：用户账户、权限、流水、分类、账户、账本、预算和周期事项均已有模块仓储；用户账户、预算与周期事项进程缓存及其兼容入口已移除 |
...
1. 继续缩减 `InMemoryStore` 与 `EnterpriseStore` 的公开兼容读模型，预算与周期事项已完成数据库单一事实源改造；
