...
| P1 | `InMemoryStore` 名称与 JDBC 事实不符且责任过多 | 拆为 identity、operations、finance 仓储 | 进行中：权限用户读写和流水读写均已有模块仓储；旧存储只保留兼容读模型及尚未迁移的数据边界 |
| P1 | `AccountingService` 同时编排账户、流水、预算和报表 | 拆为独立应用用例，跨模块通过契约协作 | 进行中：流水读写已迁入 `operations`，并通过会计协作端口访问账户、分类和账本；这些数据仍待迁入所属模块 |
...
1. 将账户和账本迁入 `finance` 应用边界及模块仓储；
2. 将分类持久化迁入 `operations`，进一步收窄临时会计协作适配器；
