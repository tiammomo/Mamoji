...
| P1 | 多个接口使用 `Map<String, Object>` | 强类型 DTO、Bean Validation、统一 Problem Detail | 进行中：流水读写、分类、预算、权限、身份认证、审批、周期事项、税务事项、部门、全部票据写命令及票据列表/汇总已完成 |
...
4. 为对象存储建立容量水位、保留周期、孤立对象核对与恢复演练证据；票据—流水联动已使用最小 typed 查询契约，附件元数据已完成 V32 数据库硬化。
...
票据查询校验、数据库分页与聚合汇总见 [ADR 0015：类型化票据查询并下推数据库计算](adr/0015-type-receipt-queries-and-database-aggregation.md)。
