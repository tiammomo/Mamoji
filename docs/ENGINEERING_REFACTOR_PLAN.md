...
| P1 | `Models` 聚合多个无关可变模型 | 模型回到所属模块，优先使用不可变值对象 | 进行中：权限、预算、账户、账本、账本成员、分类和流水已迁回所属模块 |
...
1. 将 `Models` 中的用户与邀请模型迁入 `identity`；
2. 继续缩减 `InMemoryStore` 与 `EnterpriseStore` 的公开兼容读模型；
