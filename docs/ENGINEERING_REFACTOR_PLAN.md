...
| P1 | `InMemoryStore` 名称与 JDBC 事实不符且责任过多 | 拆为 identity、operations、finance 仓储 | 进行中：权限用户读写和流水只读查询已有模块仓储，旧写路径及兼容缓存仍待迁移 |
| P1 | `AccountingService` 同时编排账户、流水、预算和报表 | 拆为独立应用用例，跨模块通过契约协作 | 进行中：流水读写用例已迁入 `operations`，账户和分类仍在旧服务 |
...
1. 为流水写用例建立 `operations` 仓储端口，继续缩减 `InMemoryStore`；
2. 将账户和账本迁入 `finance` 应用边界及模块仓储；
