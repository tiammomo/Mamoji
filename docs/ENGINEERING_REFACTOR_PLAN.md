...
| P1 | `InMemoryStore` 名称与 JDBC 事实不符且责任过多 | 拆为 identity、operations、finance、budget、recurring 仓储 | 进行中：用户账户、权限、流水、分类、账户、账本、预算和周期事项均已有模块仓储；用户账户、资金账户、账本及成员、流水、预算与周期事项进程缓存、双写及其兼容入口已移除，流水由 V17、资金账户由 V18、账本及成员由 V19 强类型 schema 保护 |
...
| P1 | `Models` 聚合多个无关可变模型 | 模型回到所属模块，优先使用不可变值对象 | 进行中：身份、权限、预算、账户、账本、账本成员、分类、流水和周期事项已迁回所属模块 |
| P1 | 多个接口使用 `Map<String, Object>` | 强类型 DTO、Bean Validation、统一 Problem Detail | 进行中：流水读写、预算、权限、身份认证、审批和周期事项写命令已完成 |
...
1. 将分类迁出 `InMemoryStore`，继续缩减 `EnterpriseStore` 的公开兼容读模型；账户、账本及成员、流水、预算与周期事项已完成数据库单一事实源改造；
