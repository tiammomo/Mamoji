...
| P1 | 多个接口使用 `Map<String, Object>` | 强类型 DTO、Bean Validation、统一 Problem Detail | 进行中：流水读写、分类、预算、权限、身份认证、审批和周期事项写命令已完成 |
...
1. 继续缩减 `EnterpriseStore` 的公开兼容读模型，并把 HR、税务对象迁入各自模块仓储；分类及会计核心对象已完成数据库单一事实源改造；
