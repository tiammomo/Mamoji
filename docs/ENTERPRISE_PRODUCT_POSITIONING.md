...
- 资金账户、账本及成员、流水、分类、预算、周期事项、税务事项、部门、员工与任职事件已移除旧 Store 中的 Map 和双写，以 PostgreSQL 专属仓储为唯一事实源；流水由 V17、账户由 V18、账本及成员由 V19、分类由 V20、税务事项由 V21、部门由 V22、员工由 V23、任职事件由 V24 的类型、租户和流程约束保护。`InMemoryStore` 已不再持有业务集合，`EnterpriseStore` 中的剩余 Map 仅承载尚未拆分的公司与主体划转兼容状态。
