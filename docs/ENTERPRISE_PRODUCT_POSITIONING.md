...
- 资金账户、流水、预算与周期事项已移除旧 Store 中的 Map 和双写，以 PostgreSQL 专属仓储为唯一事实源；流水的公司归属、金额、日期、退款生命周期和跨表引用由 V17 约束保护，账户的金额、日期、状态、公司/用户归属和公司内账本引用由 V18 约束保护。其他剩余 Map 仅用于演示初始化、恢复兼容和渐进迁移，在线业务服务不直接读取这些集合。
