...
- 数据库仍保留单实例保护开关。生产启动期全表兼容修复已经删除，票据派生默认值由 V27 一次性回填，历史公司工作区由 V28 一次性补齐；新公司在业务事务内创建成员、账本和分类，账本/分类扫描仅保留在 demo 条件 Bean。V29 已为外部 Webhook 投递增加唯一租约 fencing 与稳定幂等键；完成其余双实例写入、提醒任务抢占和故障重试验证后，再把多实例作为默认生产拓扑。
