...
- 升级到 V31 前，已在备份副本确认票据的公司/用户/审批人/流水引用、金额与税率、日期、枚举、字段长度及生命周期组合满足预检；已为表重写安排维护窗口。本次发布使用默认一致性备份且未设置 `SKIP_BACKUP=true`，旧后端保持停止直至新版本完成迁移并就绪。
- 升级到 V32 前，已在备份副本确认附件指纹均引用同公司有效票据，SHA-256 可规范为唯一的小写 64 位十六进制值，basename 非空且不超过 255 字符，文件大小非负，创建时间可解析且有限。已为附件表改写安排维护窗口，并确认 PostgreSQL 与 MinIO 一致性备份可配套恢复；不得让 V31 后端与 V32 schema 重叠运行。
- 升级到 V33 后，已确认每家公司恰有一条默认开放的账期控制记录，公司创建与流水写入触发器存在；全部新后端实例就绪前未执行首次关账。已使用财务管理员演练关账、历史流水拦截和填写原因反结账，并确认只读成员无操作权限、审计和 Outbox 事件完整。
...
- 如使用应用内结构化备份，已确认格式为 `2.3`，且公司成员关系、账本成员公司范围、预算占用账本与会计期间关账水位均包含在导出数据集中；恢复会清空会话、投递、Outbox 和定时任务租约等运行态。恢复旧 `2.2`/`2.1`/`2.0` 文件时确认账期会默认开放，并已排除无公司账本、无公司分类、孤立或重复税务事项、无公司成员关系及跨公司部门引用。
- 备份目录有独立磁盘或外部对象存储同步策略。
- 已配置每日备份 cron，保留周期与公司数据恢复要求一致。
- 已确认备份维护窗口会短暂停止入口、后端写入和 MinIO，并为探针设置了合理告警延迟；部署脚本会在成功备份后保持旧入口、前端和后端停止，直到新版本启动完成。
